...
CREATE FUNCTION my_schema.get_category(_category my_schema.categories, _team INTEGER = -1)
...
CREATE FUNCTION my_schema.get_challenge(_challenge INTEGER, _team INTEGER = -1)
    RETURNS TABLE(id INTEGER, name varchar(64), category my_schema.categories, points INTEGER, description varchar(1024), flag varchar(64), solves INTEGER, team_solved BOOLEAN)
LANGUAGE SQL
AS $$
